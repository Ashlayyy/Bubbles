<template>
	<div v-if="welcomeSettings && leaveSettings">
			...
			<div class="bg-card border border-border rounded-lg p-6">
				<div class="flex items-center justify-between mb-4">
					...
					<label class="relative inline-flex items-center cursor-pointer">
						<input
							type="checkbox"
							v-model="welcomeSettings.enabled"
							class="sr-only peer"
						/>
						<div
				...
				<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
					...
					<div class="space-y-4">
						<div>
							...
							<DiscordItemSelector
								:items="channels"
								v-model:selectedIds="welcomeSettings.channelId"
								:multiple="false"
						...
						<div class="flex items-center justify-between">
							...
							<label class="relative inline-flex items-center cursor-pointer">
								<input
									type="checkbox"
									v-model="welcomeSettings.embedEnabled"
									class="sr-only peer"
								/>
								<div
									class="w-11 h-6 bg-muted rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary"
						...
						<div
							v-if="welcomeSettings.embedEnabled"
							class="space-y-4 pl-4 border-l-2 border-primary"
						>
							<div>
								<label
							...
							<div>
								...
								<input
									...
									type="color"
									class="w-full h-10 bg-background border border-border rounded-lg"
								/>
							</div>
						</div>
						...
						<div class="flex items-center justify-between">
							...
							<label class="relative inline-flex items-center cursor-pointer">
								<input
									type="checkbox"
									v-model="welcomeSettings.pingUser"
									class="sr-only peer"
								/>
								<div
									class="w-11 h-6 bg-muted rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary"
						...
						<div class="flex items-center justify-between">
							...
							<label class="relative inline-flex items-center cursor-pointer">
								<input
									type="checkbox"
									v-model="welcomeSettings.dmUser"
									class="sr-only peer"
								/>
								<div
									class="w-11 h-6 bg-muted rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary"
						...
						<div v-if="welcomeSettings.dmUser">
							...
							>
							<textarea
								v-model="welcomeSettings.dmMessage"
								class="w-full bg-background border border-border rounded-lg px-3 py-2 text-foreground"
								rows="2"
					...
					<div class="space-y-4">
						...
						<div class="bg-background border border-border rounded-lg p-4">
							<div
								...
								:style="{ borderColor: welcomeSettings.embedColor }"
							>
								<h4 class="font-semibold text-card-foreground mb-2">
									{{ welcomeSettings.embedTitle || 'Welcome!' }}
								</h4>
								<p class="text-card-foreground">
									{{
										welcomeSettings.embedDescription ||
										welcomeSettings.messages[0] ||
										'Welcome to the server!'
									}}
								</p>
								<div class="text-xs text-muted-foreground mt-2">
									{{ welcomeSettings.embedFooter || 'Member #1,234' }}
								</div>
							</div>
							<div v-else>
								<p class="text-card-foreground">
									{{ welcomeSettings.messages[0] || 'Welcome to the server!' }}
								</p>
							</div>
						</div>
						<div
							v-if="welcomeSettings.messages.length > 1"
							class="text-xs text-muted-foreground"
						>
							Showing preview of first message ({{
				...
				<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
					...
					<div class="space-y-4">
						<div>
							...
							<DiscordItemSelector
								:items="channels"
								v-model:selectedIds="leaveSettings.channelId"
								:multiple="false"
								placeholder="Select a channel..."
								itemType="channel"
							/>
						...
						<div class="flex items-center justify-between">
							...
							<label class="relative inline-flex items-center cursor-pointer">
								<input
									type="checkbox"
									v-model="leaveSettings.embedEnabled"
									class="sr-only peer"
								/>
								<div
									class="w-11 h-6 bg-muted rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary"
						...
						<div
							v-if="leaveSettings.embedEnabled"
							class="space-y-4 pl-4 border-l-2 border-primary"
						>
							<div>
								<label
					...
					<div class="space-y-4">
						...
						<div class="bg-background border border-border rounded-lg p-4">
							<div
								v-if="leaveSettings.embedEnabled"
								class="border-l-4 pl-4"
								:style="{ borderColor: leaveSettings.embedColor }"
							>
								<h4 class="font-semibold text-card-foreground mb-2">
									{{ leaveSettings.embedTitle || 'Goodbye!' }}
								</h4>
								<p class="text-card-foreground">
									{{
										leaveSettings.embedDescription ||
										leaveSettings.messages[0] ||
										'User has left the server.'
									}}
								</p>
								<div class="text-xs text-muted-foreground mt-2">
									{{ leaveSettings.embedFooter || 'We now have 1,233 members' }}
								</div>
							</div>
							<div v-else>
								<p class="text-card-foreground">
									{{ leaveSettings.messages[0] || 'User has left the server.' }}
								</p>
							</div>
						</div>
						<div
							v-if="leaveSettings.messages.length > 1"
							class="text-xs text-muted-foreground"
						>
							Showing preview of first message ({{
								leaveSettings.messages.length
							}}
							total messages)
						</div>
					</div>
...
const channels = computed(
	() =>
		guildsStore.selectedGuild?.channels
			.filter((c) => c.type === 0)
...
const save = async () => {
	try {
		await welcomeStore.saveSettings();
