...
const routes: RouteRecordRaw[] = [
  ...
  { path: '/login', name: 'Login', component: Login },
  {
    ...
    children: [
      ...
      { path: 'command-aliases', name: 'CommandAliases', component: CommandAliases },
  ...
  { path: '/command-aliases/:guildId', redirect: to => ({ path: `/server/${to.params.guildId}/command-aliases` }) },
...
router.beforeEach(async (to) => {
  ...
  // Try to check for an existing session if we don't have a user and we're not a demo user
  if (!auth.isAuthenticated && !auth.isDemoUser) {
