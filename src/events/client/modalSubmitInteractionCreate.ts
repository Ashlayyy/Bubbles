...
export default new ClientEvent("interactionCreate", async (interaction) => {
  ...
  switch (true) {
    case interaction.customId === "reaction-role-create-modal": {
    ...
    case interaction.customId.startsWith("add-reaction-modal-"): {
      ...
      // Validate custom emoji format (only :name: allowed)
      const emojiRegex = /^:[\w_]+:$/;
      if (!emojiRegex.test(emoji)) {
        await interaction.reply({
          content:
            "❌ Invalid emoji format! Please use custom emoji format like `:emoji_name:` only. Unicode emojis are not supported.",
          flags: MessageFlags.Ephemeral,
        });
        return;
      }
    ...
    default: {
      logger.error(new ReferenceError("Could not match customId of modal to one of this bot's!"));
    }
  }
});
