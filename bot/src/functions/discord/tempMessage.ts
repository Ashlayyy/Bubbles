 ...
 * @param ephemeral [default: false] Whether the message should be ephemeral (only visible to the user)
 */
export default async (
  ...
  logger.verbose(`Ticking tempMessage: ${text}`);
  logger.verbose(`Ephemeral: ${ephemeral}`);

  // Ephemeral messages cannot be edited or deleted by bots due to Discord API limitations
  if (ephemeral) {
    await interaction.followUp({
      content: text,
      flags: MessageFlags.Ephemeral,
    });
    logger.verbose("Done sending ephemeral tempMessage (no countdown possible)!");
    return;
  }
...
async function countdown(t: number, countdownIntervalInSeconds: number, tempMessage: Message, newText: string) {
  const tempMsg = await tempMessage.edit({
    content: newText + t.toString(),
  });
